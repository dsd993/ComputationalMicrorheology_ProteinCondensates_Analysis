...
int main()
{
	...
	double TimeUnitConversion, *MSD, *MSD2, aa, aa2;
	...
	for (int FileCounter = 0; FileCounter < NumberofIntervals; ++FileCounter)
	{
        ...
        MSD = new double[NumberofAtomsofInterest];
        MSD2 = new double[NumberofAtomsofInterest];
	if (COMFlag == "no") //Checking for the COM flag
	{
		...
		while (1)
		{
			...
			MSD2[AI] = 0;
				for (int FileCounter = 0; FileCounter < NumberofIntervals; FileCounter++)
				{
					MSD[AI] += CalculateSquaredDisplacement(D1[FileCounter].Atoms[IndexAtomsofInterest[AI]].AtomCoord, D1[FileCounter+n].Atoms[IndexAtomsofInterest[AI]].AtomCoord);
					MSD2[AI] += pow(CalculateSquaredDisplacement(D1[FileCounter].Atoms[IndexAtomsofInterest[AI]].AtomCoord, D1[FileCounter+n].Atoms[IndexAtomsofInterest[AI]].AtomCoord),2);
				}
			MSD[AI] /= (NumberofIntervals);
                        MSD2[AI] /= (NumberofIntervals);
			}
			
                        FinalOutput << n*IntervalTime << " " << n*IntervalTime*TimeUnitConversion << " ";
                        for (int AI = 0; AI < NumberofAtomsofInterest; AI++)
                        {			
			FinalOutput << MSD[AI] << " ";
                        }
	...
	else
	{
		...
		while (1)
		{
			...
			for (int FileCounter = 0; FileCounter < NumberofIntervals; FileCounter++)
			{
				for (int ChainCounter = 0; ChainCounter < NumberofChains; ++ChainCounter)
				{
					aa += pow((COM_EachChain[FileCounter][ChainCounter].COMx - COM_EachChain[FileCounter+n][ChainCounter].COMx),2) + pow((COM_EachChain[FileCounter][ChainCounter].COMy - COM_EachChain[FileCounter+n][ChainCounter].COMy),2);
					aa2 += pow((pow((COM_EachChain[FileCounter][ChainCounter].COMx - COM_EachChain[FileCounter+n][ChainCounter].COMx),2) + pow((COM_EachChain[FileCounter][ChainCounter].COMy - COM_EachChain[FileCounter+n][ChainCounter].COMy),2)),2);
			    }
			}
			
			aa /= (NumberofIntervals*NumberofChains);
			aa2 /= (NumberofIntervals*NumberofChains);
			
			FinalOutput << n*IntervalTime << " " << n*IntervalTime*TimeUnitConversion << " " << aa << " " << aa2 << " " << ((3*aa2)/(5*aa*aa))-1 << endl;
			n++;
			NumberofIntervals -= 1;
		}
	}
